    ...
    if nk <= 1:
        ...
        if ifmovie:
            ...
            for k in karray:
                ...
                axs[0,0].set_ylabel(r'$q$', fontsize=12)
                axs[0,0].set_xlabel(r'$\psi$', fontsize=12)
                ...
                axs[0,1].set_xlabel(r'$\psi$', fontsize=12)
                ...
                axs[1,0].set_ylabel(r'$e_r$', fontsize=12)
                axs[1,0].set_xlabel(r'$\psi$', fontsize=12)
                ...
                axs[1,1].set_ylabel(r'$e_z$', fontsize=12)
                axs[1,1].set_xlabel(r'$\psi$', fontsize=12)
                axs[1,1].set_xscale('log')
                axs[1,1].set_ylim(minimum(ez2.real, ez2.imag).min(), maximum(ez2.real, ez2.imag).max())
                # fig.suptitle(r"$\alpha = "+str(alpha)+"$, $z = {:5.2f}$".format(zlist[ctr]), fontsize=14)
                fig.suptitle(r"$z = {:5.2f}$".format(zlist[ctr]), fontsize=16)
                fig.set_size_inches(8.,6.)
                fig.tight_layout()
                savefig(ddir+'/fourpack{:05d}'.format(k)+'.png')
                close()
                ctr+=1
                # ffmpeg -f image2 -r 15 -pattern_type glob -i 'Splinetest_alpha0.5/fourpack????0.png' -pix_fmt yuv420p -b 4096k fiver.mp4
        # Ez:
        if nk < 100:
            clf()
            fig, axs = plt.subplots(2)
